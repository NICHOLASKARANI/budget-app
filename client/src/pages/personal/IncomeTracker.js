...
export default function IncomeTracker() {
  ...
  const updateMutation = useMutation(
    (data) => api.put('/income/' + editingIncome.id, data),
    {
      onSuccess: () => {
        queryClient.invalidateQueries('income');
        queryClient.invalidateQueries('income-summary');
  ...
  const getSmartInsight = () => {
    ...
    if (thisMonth && lastMonth && thisMonth.total > lastMonth.total) {
      ...
      return 'Income Alert: Your income increased by ' + increase + '% compared to last month! Great job!';
    }
    return null;
  };
              ...
              <div className="inline-block align-bottom bg-white dark:bg-gray-800 rounded-2xl px-4 pt-5 pb-4 text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full sm:p-6">
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                  ...
                  <div className="flex space-x-3 pt-4">
                    <button type="submit" className="flex-1 py-3 bg-indigo-600 text-white rounded-xl hover:bg-indigo-700 transition-colors">Save</button>
                    <button type="button" onClick={() => { setShowForm(false); setEditingIncome(null); resetForm(); }} className="flex-1 py-3 bg-gray-200 dark:bg-gray-700 text-gray-700 dark:text-gray-300 rounded-xl hover:bg-gray-300">Cancel</button>
                  </div>
                </form>
              </div>
            </div>
