...
export default function AnnualSummary() {
  ...
  const handleExport = (format) => {
    ...
    const filename = 'annual_report_' + selectedYear;
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100 dark:from-gray-900 dark:to-gray-800 py-6 px-4">
      <div className="max-w-7xl mx-auto">
        <div className="flex justify-between items-center mb-6">
          <div>
            <h1 className="text-3xl font-bold text-gray-900 dark:text-white">Annual Summary</h1>
            <p className="text-gray-600 dark:text-gray-400">Year in review - {selectedYear}</p>
          </div>
        ...
        <div className="grid grid-cols-4 gap-4 mb-8">
          ...
          <div className="bg-white dark:bg-gray-800 rounded-2xl p-6 shadow-lg">
            <p className="text-gray-500 dark:text-gray-400">Net Savings</p>
          ...
          <table className="w-full">
            <thead className="bg-gray-50 dark:bg-gray-700">
              <tr>
                ...
                <th className="p-3 text-left">Savings Rate</th>
              </tr>
            </thead>
            <tbody>
              {summary?.monthly?.map(row => {
                const savings = (row.income || 0) - (row.expenses || 0);
                const rate = row.income > 0 ? ((savings / row.income) * 100).toFixed(1) : 0;
                return (
                  <tr key={row.month} className="border-t dark:border-gray-700">
                    <td className="p-3">{row.month}</td>
                    <td className="p-3">{user?.currency} {(row.income || 0).toLocaleString()}</td>
                    <td className="p-3">{user?.currency} {(row.expenses || 0).toLocaleString()}</td>
                    <td className="p-3 text-green-600">{user?.currency} {savings.toLocaleString()}</td>
